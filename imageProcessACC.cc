...
#pragma acc data copyin(im    [:width * height])\
				 create(NR    [:width * height])\
	             create(G     [:width * height])\
	             create(phi   [:width * height])\
	             create(Gx    [:width * height])\
	             create(Gy    [:width * height])\
	             create(pedge [:width * height])\
                 copyout(image_out [:width * height])
	{
		// Noise reduction
...
#pragma acc data present(NR[:width * height]) \
                 ...
                 present(Gx[:width * height]) \
                 present(Gy[:width * height])
#pragma acc parallel loop independent collapse(2)
	for(int i=0; i<height; i++) {
		for(int j=0; j<width; j++) {
			if (i >= 2 && j >= 2 && i < (height - 2) && j < (width - 2)) {
